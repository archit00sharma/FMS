...
const userService = (fastify) => {
    ...
    const create = async (data) => fastify.prisma.user.create({
        data
    }).catch((err) => {
        throw err;
    });

    const read = async (cond = {}) => fastify.prisma.user.findFirst({
        where: { is_deleted: false, ...cond },
    }).catch((err) => {
        throw err;
    });

    const update = async (id, data) => fastify.prisma.user.update({
        where: { id },
        data,
    }).catch((err) => {
        throw err;
    ...
    const all = async (page_no, no_of_records, search, cond = {}) => {
        ...
        return fastify.prisma.user.aggregateRaw({ pipeline }).catch((err) => {
            throw err;
        });
    };
